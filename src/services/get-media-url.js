 ...
 * @return {string} - The URL to the asset, or an empty string on failure.
 */
const getMediaUrl = ( media, size = 'full' ) => {
	const {
		media_details: {
			sizes: {
				[ size ]: { source_url: firstChoice = '' } = {},
				full: { source_url: thirdChoice = '' } = {},
			} = {},
		} = {},
		sizes: {
			[ size ]: { url: secondChoice = '' } = {},
			full: { url: fourthChoice = '' } = {},
		} = {},
